...
import LSDCameraCaptureModal from "../components/LSDCameraCaptureModal";
import {
  MODULE_META,
  DiseaseInfoPanel,
  CowSelector,
...
export default function LSDDetectionPage() {
  ...
  // Snapshot handed back from the live camera modal
      ...
      >
        <Card className="p-6 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* Photo Input — upload a file or take a live photo */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">
                  {t("detectionForms.uploadLSDPhoto") || "Skin or Body Photograph"}{" "}
                  <span className="text-red-500">*</span>
                </label>
                <span className="text-[11px] font-medium text-slate-400 dark:text-slate-500">
                  {t("detectionForms.uploadLSDSubtitle") || "Clear photo showing visible nodules or skin lesions (PNG, JPG)"}
                </span>
              </div>
              ...
              {imagePreview ? (
                <div className="relative rounded-2xl overflow-hidden border-2 border-slate-200 dark:border-slate-700 bg-slate-900 group">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-violet-600 text-white text-xs font-bold hover:bg-violet-700 transition inline-flex items-center gap-1.5"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <button
                    type="button"
                    onClick={() => setIsCameraOpen(true)}
                    className="flex flex-col items-center justify-center p-6 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-violet-500 dark:hover:border-violet-500 bg-slate-50 dark:bg-slate-800/40 hover:bg-violet-50/30 dark:hover:bg-violet-950/20 transition-all text-center group cursor-pointer"
                  >
                    <div className="h-10 w-10 rounded-full bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-400 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
