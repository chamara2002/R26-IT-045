...
/**
 * Live camera capture modal for the Lumpy Skin Disease (LSD) module only.
 *
 * A dedicated copy of LiveCameraCaptureModal so the LSD flow gets skin/nodule
 * wording and the violet module theme without altering the shared udder modal
 * used by the mastitis flow.
 */
export default function LSDCameraCaptureModal({ isOpen, onClose, onCapture }) {
  const { t } = useI18n();
  ...
  const handleCaptureSnapshot = () => {
    ...
    // If using front camera, mirror image for a natural snapshot
    if (facingMode === "user") {
    ...
    canvas.toBlob(
      (blob) => {
        if (!blob) return;
        const file = new File([blob], `lsd_camera_${Date.now()}.jpg`, {
          type: "image/jpeg",
        ...
        >
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-bold text-white tracking-wide">
                  {t("lsdCameraModal.title") || "Live Skin / Body Camera"}
                </h3>
                <p className="text-[11px] text-slate-400">
                  {t("lsdCameraModal.subtitle") || "Frame the affected skin area and tap capture"}
                </p>
              </div>
            </div>
          ...
          <div className="relative w-full aspect-4/3 sm:aspect-16/9 bg-black flex items-center justify-center overflow-hidden">
            ...
            {!isLoading && !cameraError && (
              <div className="absolute inset-8 sm:inset-12 pointer-events-none border-2 border-dashed border-violet-400/70 rounded-3xl flex flex-col justify-between p-4 shadow-[0_0_20px_rgba(124,58,237,0.15)]">
                <div className="flex justify-between text-[10px] font-mono text-violet-300/90 tracking-widest uppercase">
                  <span>┌ Skin / Nodule Frame</span>
                  <span>┐</span>
                </div>
                <div className="text-center">
                  <span className="px-3 py-1 rounded-full bg-black/60 backdrop-blur-xs text-white text-[11px] font-semibold">
                    Center visible nodules or lesions
                  </span>
                </div>
                <div className="flex justify-between text-[10px] font-mono text-violet-300/90 tracking-widest uppercase">
                  <span>└</span>
          ...
          <div className="p-4 sm:p-5 bg-slate-900 border-t border-slate-800 flex items-center justify-between gap-4">
            ...
            <button
              ...
              disabled={isLoading || Boolean(cameraError)}
              className="h-14 w-14 sm:h-16 sm:w-16 rounded-full bg-gradient-to-tr from-violet-500 to-fuchsia-400 p-1 shadow-lg shadow-violet-500/30 hover:scale-105 active:scale-95 transition-all disabled:opacity-40 disabled:hover:scale-100 cursor-pointer flex items-center justify-center"
